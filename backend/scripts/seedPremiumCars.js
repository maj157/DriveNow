...
// Function to seed premium cars to the database
async function seedPremiumCars() {
  console.log("Checking for existing premium cars...");

  try {
    // Check for existing cars to avoid duplicates
    ...
    snapshot.forEach((doc) => {
      ...
      existingCars[key] = true;
    });

    console.log(`Found ${Object.keys(existingCars).length} existing cars`);

    let addedCount = 0;
    let skippedCount = 0;

    // Add each premium car if it doesn't already exist
    for (const car of premiumCars) {
      const carKey = `${car.brand}-${car.model}`;

      if (existingCars[carKey]) {
      ...
      console.log(`Added ${car.brand} ${car.model} (${car.group})`);
      addedCount++;
    }
    ...
    console.error("Error seeding premium cars:", error);
    process.exit(1);
  }
}
